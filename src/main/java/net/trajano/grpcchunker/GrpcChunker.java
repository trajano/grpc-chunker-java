...
public final class GrpcChunker {
  ...
  /**
   * Assembles one stream object from the chunks. This has a side effect of modifying the metaChunk
   * reference with the next meta chunk or null.
   *
  ...
  public static <T, R> void chunk(
      Stream<T> input, Function<T, Stream<R>> mapper, StreamObserver<R> responseObserver) {

    input.flatMap(mapper).forEach(responseObserver::onNext);
  }

  /**
   * Given an input stream of objects, it will create chunks for the response observer. This
   * separates the concerns of the meta from the data.
   *
   * @param input stream
  ...
  public static <T, R> void chunk(
    ...
    chunk(
        input,
        e -> Stream.concat(Stream.of(metaMapper.apply(e)), dataMapper.apply(e)),
        responseObserver);
  }
  ...
  public static <T, R> Stream<T> dechunk(
    ...
    final var seed =
        assembleOne(lastChunkRef, chunkIterator, metaPredicate, objectSupplier, combiner);
  ...
  /**
   * Dechunks a GRPC stream from the request and calls the consumer when a complete object is
   * created. This stops further processing once an error has occurred.
   *
   * @param metaPredicate predicate that returns true if it is a meta chunk indicating a start of a
   *     new object.
   * @param objectSupplier this function gets the meta chunk and supplies a new object
  ...
  public static <T, R, S> StreamObserver<R> dechunkingStreamObserver(
    ...
    return new DechunkingStreamObserver<>(
        metaPredicate, objectSupplier, combiner, consumer, null, responseObserver);
  }

  /**
   * Dechunks a GRPC stream from the request and calls the consumer when a complete object is
   * created. This stops further processing once an error has occurred. This takes in a finalizer
   * {@link Consumer} which may call {@link StreamObserver#onNext(Object)} to provide a single value
   * to the stream before completion.
   *
   * @param metaPredicate predicate that returns true if it is a meta chunk indicating a start of a
   *     new object.
   * @param objectSupplier this function gets the meta chunk and supplies a new object
  ...
  public static <T, R, S> StreamObserver<R> dechunkingStreamObserver(
    ...
    return new DechunkingStreamObserver<>(
        metaPredicate, objectSupplier, combiner, consumer, finalizer, responseObserver);
  }
  ...
  /**
   * Dechunks a GRPC stream from the request and calls the consumer when a complete object is
   * created. This stops further processing once an error has occurred. This takes in a countdown
   * latch that is countdown when the processing completes or errors out.
   *
   * @param metaPredicate predicate that returns true if it is a meta chunk indicating a start of a
   *     new object.
   * @param objectSupplier this function gets the meta chunk and supplies a new object
  ...
  public static <T, R> StreamObserver<R> dechunkingStreamObserver(
    ...
    return new LatchedDechunkingStreamObserver<>(
        metaPredicate, objectSupplier, combiner, consumer, latch);
  }
}
